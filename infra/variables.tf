...
variable "scrape_lambda_memory_mb" {
  type        = number
  default     = 4096
  description = "The scrape-fast Lambda re-polls every known board and upserts into a growing SQLite DB, heavier than the read-only API Lambda's workload. Bumped 512->1024 once already (a run measured 510/512MB used, Comeet's re-poll upserting a 100MB+ jobs.db); 1024->2048 on 2026-09-08 after the same overnight company-count growth pushed a real run to Runtime.OutOfMemory at 1024MB. Bumped again 2048->4096 the same day: a real run at 358 known companies measured 1367-1384MB used (67% of 2048), and the discovery pipeline (discover-companies.yml + merge-discovered-companies.yml, now running continuously rather than a one-time overnight batch) is projected to more than double that count as its queue drains -- extrapolating the same ratio forward lands close to 2048MB again, so this needs headroom for where the count is going, not just where it stood the last time this was measured. Lambda's network throughput scales with memory too, so this also helps the timeout margin below, not just safety. Shared with scrape_workday_lambda.tf's own function -- that one only handles a dozen companies and was nowhere near either wall, so this is more headroom than it strictly needs, but not worth a second variable just to avoid over-provisioning a Lambda this cheap to run either way."
}
