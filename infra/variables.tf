...
variable "lambda_memory_mb" {
  type        = number
  default     = 3008
  description = "Was 256 (\"SQLite reads on a ~2MB DB are light\") until 2026-09-08: jobs.db grew to ~197MB via the overnight Common-Crawl merge, and /api/stats' own heavy aggregates (median age, ghost-job rate, 14-day daily history) started timing out outright at 10s against a DB nearly 100x the size this Lambda was sized for. Bumped 1024->2048 the same day after jobs.db grew again, to 639MB in one large discovery-pipeline batch (Max Memory Used was already 700-720MB against the smaller cached version) -- real margin above a number that's proven it can jump hundreds of MB in a single merge, not just today's usage. More memory also means more CPU/network allocation in Lambda, directly helping the query speed itself, not just headroom. EDIT 2026-09-16: 2048, up from 1536. Max Memory Used had reached 1516MB, which is 98.7% of the ceiling, and that is not headroom on the function serving every request. Over 6h: Duration median 74ms but p90 13,127ms and max exactly on the timeout, with 1 timeout and 7 errors in 1,465 invocations, and a real 500 on /api/jobs. The cost is the cold-start download of jobs-read.db, 958MB and still growing with the discovery queue. 2048 rather than 3008 on purpose: Lambda scales CPU and network with memory, so a bigger number would shorten that download further, but most invocations are warm 74ms reads and this function has already been the largest line on the bill at roughly 20,000 GB-seconds a day, so doubling the common case to speed the rare one is the wrong trade. db.py's TransferConfig stays at max_concurrency=2 in the same change, deliberately: raising memory and concurrency together would make it impossible to tell which one moved the number. Measure p90 and Max Memory Used before touching concurrency. This variable has exactly one consumer, aws_lambda_function.api; the scrape and auth functions carry their own. EDIT 2026-09-22: 4096, up from 2048, during a live outage. /api/jobs?search=python returned HTTP 500 after 29.4s, search=engineer 200 after 29.3s, and even a term with no results took 14.6s. The 2026-09-16 edit above named the cause and it has only got worse: the snapshot was 958MB then and is over 2GB now, and lambda_timeout_s is already at 29, the whole of the API Gateway ceiling, so there is no room left there. Measured before making this change: roughly 20,000 GB-seconds a day over about 6,000 invocations, which is a mean duration of 1.67s against a median of 74ms. Solving for the split puts cold starts at about 8% of invocations and about 96% of all compute. So this function does not cost money to answer queries, it costs money to download a 2GB file 480 times a day. That is why doubling memory is close to free rather than double: a cold start is network-bound, Lambda scales network and CPU with memory, and twice the memory for half the duration is the same GB-seconds. Modelled at +4% if the halving holds and +100% if it does not, which is 10 dollars a month to 20 dollars a month. The worst case is a sandwich. CORRECTION, same day: 4096 was rejected. terraform apply failed with ValidationException saying MemorySize must be less than or equal to 3008, so this account is capped at 3008 until AWS raises the quota. 3008 is therefore the whole of the remaining room, the same way lambda_timeout_s at 29 is. That is +47% rather than the +100% the model assumed, so expect the cold start to fall by about a third, not by half. If that is not enough the next levers are a service quota increase request, raising TransferConfig concurrency, or making the snapshot smaller, and only the last one actually fixes it. db.py TransferConfig max_concurrency stays at 2 in this change, deliberately, for the same reason the 2026-09-16 edit gave: two levers at once and neither can be attributed. Raise it once CloudWatch shows Max Memory Used against the new ceiling. None of this is the real fix. The real fix is that the API downloads descriptions it does not need to list jobs: DESCRIPTION_MAX_CHARS is 8000 and there are 800k rows, so the descriptions are most of the 2GB. Splitting them into their own S3 object, with an offset and length in the list DB and a ranged GET per job page, is what stops this recurring at 4GB and again at 8GB."
}

variable "lambda_timeout_s" {
  type        = number
  default     = 29
  description = "Was 10. Bumped to 25, not higher: API Gateway v2 (HTTP API, see infra/apigateway.tf) has a hard 29-30s integration timeout that Terraform can't raise -- a Lambda timeout past that ceiling would just mean API Gateway itself cuts the request instead, no better outcome. If real query duration keeps growing past this as the merge queue keeps landing, the fix is making /api/stats' own aggregates cheaper (pre-computed, not live on every request), not another timeout bump into a wall that doesn't move. EDIT 2026-09-16: 29, on request, which is the whole of the remaining room. The live integration confirms the ceiling at 30000ms, so this buys four seconds and no more. Why it was needed: /api/jobs returned a 500 to a real request, and over 6h there was 1 timeout and 7 errors in 1,465 invocations, with Duration median 74ms but p90 13,127ms and max sitting exactly on the old 25,000ms limit. The cause is NOT the aggregates named above, which are precomputed now. It is the cold-start snapshot download: jobs-read.db is 958MB and db.py fetches it at TransferConfig(max_concurrency=2), a setting chosen to keep the download buffer small because Max Memory Used already sits at 1516MB against lambda_memory_mb's 1536. So the download is deliberately slow and four seconds only moves the cliff, it does not remove it. The lever that would actually fix this is memory: Lambda scales CPU and network with it, and more headroom would also allow a higher transfer concurrency. That is a bigger change than a timeout bump and was not made here."
}

variable "scrape_fast_memory_mb" {
  type        = number
  default     = 1536
  description = "Re-sharded 2026-09-08 to decouple probe.py's own workload from total company count -- that part worked (a shard's own ATS-fetching stays cheap and fast regardless of how many companies exist overall). Missed at the time: load_to_sqlite.py's pull-modify-push cycle downloads and opens the FULL jobs.db on every single invocation no matter how small the shard is, so sharding never touched THIS Lambda's real growth exposure at all. Confirmed live the same day: jobs.db passed 795MB and every single scrape_fast invocation started failing outright (Runtime.OutOfMemory or a straight timeout) at the 1024MB/120s this had been sized to -- a live, total-pipeline outage, not a slow degradation. Bumped to 3008MB (this account's real Lambda memory ceiling) alongside the timeout bump below. This Lambda has the exact same unaddressed growth exposure as scrape_workday now -- watch both as jobs.db keeps growing; sharding was only ever half the fix. EDIT 2026-09-10: 1536, down from 3008. The OOM history above is real but it is history: this Lambda stopped opening a database when delta fragments replaced the pull-modify-push, and it now runs probe.py and nothing else. Measured over 24h, Max Memory Used peaks at 871MB, so 3008 was paying for a shape this function no longer has. 1536 keeps 76% headroom over the real peak. Lambda scales CPU with memory, so this is only a saving if the work is waiting on network rather than on itself: 3,450 mostly-304 HTTP fetches says it is, but watch Duration after this ships and put it back if the sweep gets slower rather than cheaper. Measured at 1536: 42.3s against 50.5s at 3008, peak 638MB. It got faster on less memory, which settles the question, so 1024 next. Then back up to 1280: on arm64 a sweep peaked at 891MB, and 13% headroom on the function that feeds everything else is not headroom. Peak here tracks how many boards changed, not how many were polled, so it moves. EDIT, measured over a full hour rather than a handful of runs: 1280 was a mistake. At 1536 the sweep averages 44.1s; at 1280 it averages 81.2s over 12 runs, min 66 and max 102. That is 102 GB-seconds a run against 66, so the smaller setting costs 53% MORE. Peak sits at 987MB against a 1280MB ceiling, and the likely cause is pressure at 77% of the limit rather than the CPU that comes with the memory. Back to 1536, where both the duration and the headroom are better. EDIT 2026-09-14: 1152, down from 1536. The sweep no longer opens a database, it only polls and writes fragments, so it is network-bound: Max Memory Used peaked at 974MB over 48h (p99 888MB, runs since the streaming change 506MB). 1152 keeps about 18% over that peak and cuts the function's GB-seconds by a quarter. EDIT 2026-09-14, later: back to 1536. Lambda gives CPU in proportion to memory, and at 1152 the sweep parsed large boards (SpaceX 2,421 jobs, Anduril 2,252) slowly enough that a busy hour pushed it past its time limit, which put it in a loop that failed every run for 45 minutes. The memory was never the constraint; the CPU that came with it was."
}
