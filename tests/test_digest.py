...
# Structure and brand
check("a compact header: the site's own mark, the wordmark, a 2px green rule",
      "OpenTechJobs</td>" in h and 'src="https://opentechjobs.org/favicon-32.png"' in h and "border-bottom:2px solid #609966" in h)
check("only DESIGN.md colours, both modes", all(c in h for c in ("#f2f0ef", "#40513b", "#609966", "#3f6f45", "#d2cfcb", "#17181c", "#ededec", "#2fae60"))
      and "#ffffff" not in h and "#dce8d4" not in h)
...
check("the calls to action are links, not buttons", "View all 4 matches &rarr;" in h and "View all matches &rarr;" in h and "border-radius" not in h)
...
# Rows
check("the title is the link, in ink, and Apply is the green",
      'font-weight:700; color:#40513b; text-decoration:none;">Senior Product Manager</a>' in h
      and 'style="color:#3f6f45; font-weight:700; text-decoration:none;">Apply &rarr;</a>' in h)
check("a company mark in the logo band, or a lettered square of the same size when there is none",
      'src="https://wix.com/favicon.png" width="28" height="28"' in h and 'line-height:28px; color:#40513b;">W</td>' in h)
...
check("hairline under each row, no bar, no box", 'width="4"' not in h and h.count("border-bottom:1px solid #d2cfcb") >= 5 and "border-radius" not in h)
